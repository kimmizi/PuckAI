...
class ActorCritic(nn.Module):
    def __init__(self, state_dim, action_dim, n_latent_var_actor, n_latent_var_critic, has_continuous_action_space,
                 action_std_init, network_depth_actor, network_depth_critic):
        ...
        if has_continuous_action_space:
            self.action_layer = nn.Sequential(
                nn.Linear(state_dim, n_latent_var_actor),
                nn.LayerNorm(n_latent_var_actor),
                nn.Tanh(),
                nn.Linear(n_latent_var_actor, n_latent_var_actor),
                nn.LayerNorm(n_latent_var_actor),
                nn.Tanh(),
                nn.Linear(n_latent_var_actor, action_dim),
        ...
        self.value_layer = nn.Sequential(
            nn.Linear(state_dim, n_latent_var_critic),
            nn.LayerNorm(n_latent_var_critic),
            nn.Tanh(),
            nn.Linear(n_latent_var_critic, n_latent_var_critic),
            nn.LayerNorm(n_latent_var_critic),
            nn.Tanh(),
            nn.Linear(n_latent_var_critic, 1)
        )

    def set_action_std(self, new_action_std):
        if self.has_continuous_action_space:
            self.action_var = torch.full((self.action_dim,), new_action_std * new_action_std).to(device)
            self.log_std.data.fill_(torch.log(torch.tensor(new_action_std)))
            self.log_std = nn.Parameter(torch.clamp(self.log_std, min=-2, max=1))
